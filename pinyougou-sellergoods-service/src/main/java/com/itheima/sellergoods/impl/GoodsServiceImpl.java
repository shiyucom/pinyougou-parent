...
//-------------------------------------------------------------------------------------------------------------
    ...
    public void add(Goods goods) {
        ...
        goodsMapper.insert(goods.getGoods()); //插入商品的状态
        //要想得到goods的id 必须 获取将扩展信息的id设置为 goods的id
        //也就是将上面的id给下面，然后下面将id 保存
        //将商品基本表的d给商品扩展表
...
//插入sku列表数据
    ...
    private void setItemValues(TbItem item, Goods goods) {
        ...
        // 分类名称
        TbItemCat itemCat = itemCatMapper.selectByPrimaryKey(goods.getGoods()
                .getCategory3Id());
        ...
        if (imageList.size() > 0) {
            item.setImage((String) imageList.get(0).get("url"));  //去集合里的第一个就行了
        }
